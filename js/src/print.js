...
const providerWithZone = new WebTracerProvider({
  resource: new Resource({
    [SEMRESATTRS_SERVICE_NAME]: {"test": 'xml-http-web-service', "key2": [1,2,3]}
  })
});
...
const collectorOptions = {
  timeoutMillis: 15000,
  url: '172.17.0.2:4318/v1/traces', // url is optional and can be omitted - default is http://localhost:4318/v1/traces
  headers: {
    foo: 'bar'
  }, // an optional object containing custom headers to be sent with each request will only work with http
  concurrencyLimit: 10, // an optional limit on pending requests
};
providerWithZone.addSpanProcessor(new SimpleSpanProcessor(new OTLPTraceExporter(collectorOptions)));
